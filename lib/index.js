...
//			returnThis.config.toComplete = [cloneRepo,getPathTree,grabFolders,writeToDisk];


function Docs(){

	this.createTemplate = function(docsObjects,callback){
		...
		var template = function(writeFileObject,afterFileWrite) {
			...
			var afterWriteCB = function(err) {
				...
				// TODO: make this return a more useful status than fine
					if (err)
						return returnWriteStatus({type:'markup',err:err})
					else {
						//console.log('saved to disk yall')
						return returnWriteStatus(null,'fine')
					}
				};
			...
			var saveToDisk = function(err,html){
				if (err){
					...
					return returnWriteStatus({type:'conversion',err:err})
				} else {
					//console.log('theHTML',html,'\n\nThe object:',writeFileObject);
					fs.outputFile(writeFileObject.fullPathAndFileName, html, afterWriteCB);
				}

			}

			//console.log('saving:',writeFileObject.fullPathAndFileName)
			convertToHTML(writeFileObject.templateHTML, utils.markedOptions, saveToDisk)
		...
		var writeTemplates = function(currentDocsObject){
			...
			//console.log('Im in the writeTemplates section',currentDocsObject.populatedSections)
			...
			for (section in sendTheseToBeWritten)
			...
			var afterSectionWrite = function(sectionResults){
				if (docSectionsToWrite.length === 0) {
					...
					//console.log('Done writing final section\n\n',currentDocsObject,'\n\n');
					if (errors.length)
			...
			//console.log('Writing THese',docSectionsToWrite);
...
*/
			...
			var doSectionSplit = function(docSectionObject,cb){
				...
				splitFiles.forEach(function(v, i) {
						...
						try {
							...
							console.log('MD Split Error in ',i,':',mdSplitError);
					...
					//console.log('making dir:',sectionPath+'/'+sectionNameNoSpace);
						...
						//			siteMap.pages.push(dirNameInRepo+'/'+getFileName);
		...
		var getPathTree = function(currentDocsObject){
			...
			jsonDir.dir2json(currentDocsObject.config.__docsRepoOnDisk,{ attributes: ['content', 'mode'] },gotTree)
		};
		...
		var cloneRepo = function(currentDocsObject){
			...
			var tmpDir = process.cwd()+'/.tmp/docTemplater/';
			if (!fs.existsSync(tmpDir))
				fs.mkdirsSync(tmpDir);

				process.chdir(tmpDir)

			var doClone = function(err){

				console.log('cloning Repo:',currentDocsObject.config.docsGitRepo);

				var clone = spawn('git', ['clone', currentDocsObject.config.docsGitRepo]);
			...
			// Find the base git repo on disk then deletet it and reclone.

			if (currentDocsObject.config.__baseDocsRepoOnDisk)
				var __docsRepoOnDisk = currentDocsObject.config.__baseDocsRepoOnDisk;
			else
				var __docsRepoOnDisk = currentDocsObject.config.__docsRepoOnDisk;


			if (fs.existsSync(__docsRepoOnDisk)){
				//console.log(tmpDir+__docsRepoOnDisk,'exists')
				fs.remove(tmpDir+__docsRepoOnDisk,doClone);
			} else {
				//console.log(tmpDir+__docsRepoOnDisk,'doesnt exist')
				doClone()
			}
		...
		var prepareThem = function(singleDocsObject){
			...
			var returnThis = {config:{errors:[],results:[]}};
	...
	//console.log('finished preparing',returnThis.config.__docsRepoOnDisk)
		...
		var afterEachStep = function(newErrors,currentDocsObject){
			...
			if (functionArray.length){
				...
				return perDocsObjectCallback()
			}
		};

		var perDocsObjectCallback = function(){
			console.log('You have completed a docs object. wtg')
