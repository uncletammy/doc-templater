		...
		var template = function(writeFileObject,afterFileWrite) {
			...
			var callBeforeConvert = function(currentWfObject,cb){
				try {
					...
					return returnWriteStatus({type:'beforeConvert',err:beforeConvertError})
				}
			};

			var callConvert = function(currentWfObject,cb){
				convertToHTML(currentWfObject.templateHTML, utils.markedOptions, function(err,convertedMarkup){
					if (err){
			...
			var callAfterConvert = function(currentWfObject,cb){
				try {
					...
					return returnWriteStatus({type:'afterConvert',err:afterConvertError})
				}
			};
