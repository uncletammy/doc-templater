...
Compiler.prototype.build = function(docsObjects, finalCB) {
  ...
  var usageErrors = _.reduce(docsObjects, function (memo, instruction, i) {
    if (typeof instruction !== 'object' || !instruction.docsGitRepo) {
      memo.push(new Error('Invalid usage: Missing `docsGitRepo` (repo src URL) in instruction #'+i+': '+util.inspect(instruction)));
    }
    return memo;
  }, []);
  if (usageErrors.length) {
    return finalCB(new Error('Invalid usage.  Errors:\n' + util.inspect(usageErrors)));
  }
