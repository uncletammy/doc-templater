...
Compiler.prototype.build = function(docsObjects, finalCB) {
  ...
  var makeJsonMenu = function(currentDocsObject){
      if (currentDocsObject.config.saveJsonMenu){
          ...
          console.log('Arranging',results.length,'total templates')

          // Full path of every menu or submenu
          var allTemplates = results;
          var allParents = _.where(allTemplates,{isParent:true});
          var allKiddos = _.where(allTemplates, {isParent:undefined});
          // var parentPaths = _.pluck(allParents,'fullPathAndFileName');

          console.log(allTemplates.length,'=',allParents.length,'+',allKiddos.length)
          ...
          var giveChildren = _.each(allParents,function(o){
              ...
              // console.log('Search by',searchBy,'for all but',parentFolderName);
              ...
              var getChildren = _.where(allKiddos,function(templateObject){
                  ...
                  if (searchIndex >- 1 && templateObject.templateTitle !== parentFolderName){
                      // console.log(searchBy,'is in',templateObject.fullPathAndFileName)
                      return templateObject             
                  }

              })

              var giveKiddosParents = _.each(getChildren,function(p){
              ...
              thisParent.children = _.pluck(getChildren,'fullPathAndFileName');
          ...
          fs.outputJson(currentDocsObject.config.saveJsonMenu, finishedTemplates, function(err) {
              if (err)
