...
Compiler.prototype.build = function(docsObjects, finalCB) {
  ...
  var writeTemplates = function(currentDocsObject) {
    ...
    var writeThisSection = function(arrayOfTemplateObjects, afterSectionWrite) {
      ...
      var afterFileWrite = function(err, writeResult) {
        if (err)
          ...
          currentDocsObject.config.jsonMenu[writeResult.templateSectionName].contents.push(writeResult);
        }
    ...
    var sendFilesToWrite = function(currentDocsObject) {
        console.log(currentDocsObject);

        var newFile = {
          templateConfig: {},
          templateTitle: getFileName,
          templateSectionName: sectionName,
          fullPathAndFileName: docSectionObject.savePath + '/' + getFileName + '.html',
  ...
  var splitMDsIntoTemplates = function(currentDocsObject) {
    ...
    var errors = [];
    currentDocsObject['populatedSections'] = {};
    currentDocsObject.config.jsonMenu = {};

    var sectionsToSplit = currentDocsObject.createTheseSections;

    /**
     * [doSectionSplit description]
     * @param  {[type]} docSectionObject [description]
     * @param  {Function} emitFile
     * @return {[type]}                  [description]
     */
    var doSectionSplit = function(docSectionObject, emitFile) {
      //self.log('\n\ndocSectionObject:',docSectionObject,'\n\n')
      //var sectionPathOnDisk = docSectionObject['-path'];

      // If `dontSplitFiles` is set, just create a single-item array.
      var splitFiles;
      if (currentDocsObject.config.dontSplitFiles) {
        splitFiles = [docSectionObject['-content']];
      } else {
        splitFiles = docSectionObject['-content'].split(/\n{0,1}[^#]#\s/ig);
      }
      ...
      splitFiles.forEach(function(v, i) {
        ...
        try {
          ...
          self.log('MD Split Error in section:', docSectionObject, ':', mdSplitError);
        ...
        if (i === 0) {
          ...
          sectionName = sectionName.replace(/^\s/ig, '');

          sectionNameNoSpace = sectionName.substring(0).replace(/\s/ig, '');

          //fs.mkdirsSync(docSectionObject.savePath);

          currentDocsObject.populatedSections[docSectionObject.savePath] = [];

          currentDocsObject.config.jsonMenu[sectionName] = {
            name:sectionName,
            contents:[]
          };
        ...
        if (getFileName.match(/[\(]/)) {
          try {
            ...
            self.log('Problem grabbing fileName in ', b, ':', grabFileNameError);
          }
        }
        ...
        currentDocsObject.populatedSections[docSectionObject.savePath].push(newFile);

        if (i === splitFiles.length - 1) {
          //self.log('on',splitFiles.length-1,'of',i,'of section',sectionName)
          return emitFile(newFile);
        }
    ...
    doSectionSplit(sectionsToSplit.shift(), function afterEachSplit (newFileObject) {
      if (sectionsToSplit.length === 0) {
        //self.log('Done splitting all section.  Thank the lord');
        ...
        //self.log(thisDocsObject.populatedSections[newFileObject.templateSectionName]);
  ...
  var returnResults = function(err, stuffCreated) {
    ...
    if (err.length) {
      sendErrors = err;
      self.log('There was one or more errors processing docs...', err);
      //return finalCB(err)
    }
    self.log('Done parsing',repoURL);
  ...
  var grabFolders = function(currentDocsObject) {
      ...
      var writeOne = function(writeFileObject){
        template(docSectionArray.shift(), afterFileWrite);

      }

      var allDone = function(){
          self.log('\n\nI found all',allMarkdownFiles.length,'markdown files in',currentDocsObject.config.__derivenRepoName);
          self.log(allMarkdownFiles);

          
          // currentDocsObject.createTheseSections = allMarkdownFiles;
          // return afterEachStep(null, currentDocsObject);
      }
          ...
          for (var key in currentDirectoryObject) {
              if (ignoreKeys.indexOf(key) < 0){            
                  if (key.toLowerCase().indexOf('.md') >= 0) {
                      ...
                      console.log('splitting',getPath,'by',currentDocsObject.config.__derivenRepoName)
                      var splitPathByRepoName = getPath.split(currentDocsObject.config.__derivenRepoName);
                          ...
                          console.log('Saved path as ',getPath)
                      //}


                      currentDirectoryObject[key]['savePath'] = getPath;
                      currentDirectoryObject[key]['mdFileName'] = mdFileName;
                      allMarkdownFiles.push(currentDirectoryObject[key]);
  ...
  var getPathTree = function(currentDocsObject) {
    ...
    //self.log('getting json tree for ',currentDocsObject.config.__docsRepoOnDisk,'\n\nconfig:',currentDocsObject.config);
  ...
  var cloneRepo = function(currentDocsObject) {
    ...
    var doClone = function(operationAndAruments) {
      ...
      gitOperation.on('close', function(code) {
        ...
        if (code === 0) {
          ...
          self.log('Sorry Homie.  The building collapsed and i\'m left holding exit code ' + code);
    ...
    if (fs.existsSync(fullGitPathOnDisk)) {
      self.log(fullGitPathOnDisk, ' exists!  Lets pull down any new changes.');
  ...
  var prepareThem = function(singleDocsObject) {
    ...
    returnThis.config.toComplete = [cloneRepo, getPathTree, grabFolders, sendFilesToWrite, writeTemplates];
    ...
    if (self.recentDRODs.indexOf(returnThis.config.parsedTemplatesDirectory) >= 0){
        self.log('Deleting',returnThis.config.parsedTemplatesDirectory);
        fs.removeSync(returnThis.config.parsedTemplatesDirectory);
    } else {
        self.log(returnThis.config.parsedTemplatesDirectory,'hasnt been previously used.');      
    }
