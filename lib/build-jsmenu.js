...
module.exports = function buildJsMenu (tree){
  ...
  tree = _.reduce(tree, function (m, templateInfo, i) {
    ...
    // `isParent` flag
    if (templateInfo.children && templateInfo.children.length >= 0) {
  ...
  // Last of all, sort the menu items alphabetically.
  // Note that this is just to prevent merge conflicts-- the actual
  // sorting (e.g. for the Sails website) happens in the code that uses
  // these files.
  tree = tree.sort(function (a,b){
    if (a.fullPathAndFileName > b.fullPathAndFileName) {
      return 1;
    }
    else if (a.fullPathAndFileName < b.fullPathAndFileName) {
      return -1;
    }
    else return 0;
  });
