...
module.exports = function ($i, cb) {
  ...
  var remoteSubPathSlug = (($i.remoteSubPath!=='./') ? ('-'+$i.remoteSubPath.replace(/\.\//g, '')) : '').toLowerCase();
  var slug = remoteSlug + '__' + branchSlug + '__' + remoteSubPathSlug;

  if (!$i.cachePath) {
    $i.cachePath = path.resolve('.tmp', slug);
  }
  if (!$i.htmlDirPath) {
    $i.htmlDirPath = path.resolve('.tmp', slug + '-html-output/');
  }
  if (!$i.jsMenuPath) {
