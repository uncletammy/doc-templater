      ...
      .exec(function(err, tree) {
        ...
        async.each(tree, function(pathToMdFile, cb) {
          ...
          if (pathToMdFile.match(/\/$/)){
            ...
            if (!!htmlSubfolderPath.match(/\/\.[^\/]/g)) return cb();

            htmlTree.push({
              templateTitle: path.basename(htmlSubfolderPath),
              fullPathAndFileName: htmlSubfolderPath
            });
            return cb();
          }
          ...
          htmlTree.push({
            ...
            fullPathAndFileName: outputHTMLFilePath
          });
          ...
          var jsmenuString;
          try {
            jsmenuString = JSON.stringify(buildJsmenu(htmlTree));
          }
          catch (e) {
            return $x(err);
          }
