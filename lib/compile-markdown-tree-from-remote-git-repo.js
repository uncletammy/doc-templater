      ...
      .exec(function(err, tree) {
        ...
        async.eachLimit(tree, 15, function(pathToMdFile, cb) {
          ...
          if (pathToMdFile.match(/\/$/)){
            ...
            relHtmlSubfolderPath = path.join(path.basename($i.htmlDirPath), relHtmlSubfolderPath);
          ...
          // else {
          //   console.log('SKIPPING:',pathToMdFile, 'because it has', path.join(theFilename,theFilename+'.md'),'... See?  ',pathToMdFile.indexOf(path.join(theFilename,theFilename+'.md')));
          // }
