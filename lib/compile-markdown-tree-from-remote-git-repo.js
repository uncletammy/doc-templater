...
module.exports = function ($i, cb) {
  ...
  : function (_onwards) {
    require('machinepack-git').pullOrClone({
      remote: $i.remote,
      branch: $i.branch,
  ...
  _pullOrClone(function (err) {
    ...
    .exec(function(err, tree) {
      ...
      async.eachLimit(tree, 15, function(pathToMdFile, cb) {
        ...
        var outputHTMLFilePath = path.join(path.dirname(relPathFromHtmlDir), path.basename(relPathFromHtmlDir, '.md')) + '.' + ($i.outputExtension||'html');
        ...
        Filesystem.read({
          ...
          success: function (markdownStr){
            ...
            $i.beforeConvert(markdownStr, function (err, markdownStr) {
              if (err) {
                _failures.push({
                  src: pathToMdFile,
                  dest: outputHTMLFilePath,
                ...
                success: function (templateMetadata){
                  ...
                  Markdown.compileToHtml({
                    ...
                    success: function (resultHtml){
                      ...
                      $i.afterConvert(resultHtml, function (err, resultHtml) {
                        if (err) {
                          _failures.push({
                            src: pathToMdFile,
                            dest: outputHTMLFilePath,
