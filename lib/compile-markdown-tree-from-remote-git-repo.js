  ...
  _pullOrClone(function (err) {
    ...
    .exec(function(err, tree) {
      ...
      async.eachLimit(tree, 15, function(pathToMdFile, cb) {
        ...
        require('machinepack-markdown').compileToHtmlFileFromMdFile({
          src: pathToMdFile,
        ...
        .exec({
          ...
          couldNotRead: function (err){
            _failures.push({
              src: pathToMdFile,
              dest: outputHTMLFilePath,
              type: 'Could not read markdown file',
              error: err
            });
            cb();
          },
          couldNotWrite: function (err){
            _failures.push({
              src: pathToMdFile,
              dest: outputHTMLFilePath,
              type: 'Could not write HTML file',
              error: err
            });
            cb();
          },
          couldNotCompile: function (err){
            _failures.push({
              src: pathToMdFile,
              dest: outputHTMLFilePath,
              type: 'Could not compile markdown file',
              error: err
            });
            cb();
          },
