...
module.exports = function (inputs, cb) {
  ...
  : function (_onwards) {
    Git.pullOrClone({
      ...
      destination: path.resolve(inputs.cachePath)
    })
    .exec(_onwards);
  };
                      ...
                      inputs.afterConvert(resultHtml, function (err, resultHtml) {
                        ...
                        Filesystem.write({
                          ...
                          success: function (){
                            ...
                            // Look up last modified date and attach it to the template metadata
                            Process.executeCommand({
                              command: 'git log -1 --format="%ai" \''+path.relative(path.resolve(inputs.cachePath), pathToMdFile)+'\'',
                              dir: path.resolve(inputs.cachePath),
                            }).exec(function (err, bufferedOutput){
                              if(err) { return cb(err); }

                              // Get a JS timestamp from the last modified date, and attach it to our template metadata.
                              templateMetadata.lastModified = (new Date(bufferedOutput.stdout)).getTime();
