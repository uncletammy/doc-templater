...
module.exports = function ($i, cb) {
  ...
  : function (_onwards) {
    M.build(require('machinepack-git/pull-or-clone'))
    .configure({
      remote: $i.remote,
      branch: $i.branch,
  ...
  _pullOrClone(function (err) {
    ...
    //   for this build step.
    M.build(require('machinepack-fs/ls'))
    .configure({
    ...
    .exec(function(err, tree) {
      ...
      async.eachLimit(tree, 15, function(pathToMdFile, cb) {
        ...
        // (used below to handle various exits of the compile-to-html-file-from-md-file machine)
        ...
        M.build(require('machinepack-markdown/compile-to-html-file-from-md-file'))
        .configure({
          src: pathToMdFile,
          dest: outputHTMLFilePath,
