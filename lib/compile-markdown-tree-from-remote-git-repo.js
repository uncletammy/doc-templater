...
module.exports = function ($i, cb) {
  ...
  if (!$i.cachePath) $i.cachePath = path.resolve('.tmp', $i.remote.replace(/[^a-zA-Z0-9]/g, '') + ( ($i.remoteSubPath!=='./') ? ('-'+$i.remoteSubPath.replace(/\.\//g, '')) : ''));
  if (!$i.htmlDirPath) $i.htmlDirPath = path.resolve('.tmp', $i.remote.replace(/[^a-zA-Z0-9]/g, '') + ( ($i.remoteSubPath!=='./') ? ('-'+$i.remoteSubPath.replace(/\.\//g, '')) : '') + '-html-output/');
  if (!$i.jsMenuPath) $i.jsMenuPath = path.join(path.dirname($i.htmlDirPath), path.basename($i.htmlDirPath)+'.jsmenu');
